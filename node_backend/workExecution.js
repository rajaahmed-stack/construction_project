...
router.get('/safety_download/:id', (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      // Combine all paths
      const allPaths = [
        ...(safety_signs ? safety_signs.split(',') : []),
        ...(safety_barriers ? safety_barriers.split(',') : []),
        ...(safety_lights ? safety_lights.split(',') : [])
      ]
        .map(p => p.trim())
      ...
      if (validPaths.length === 1) {
        return res.download(validPaths[0]);
      }
